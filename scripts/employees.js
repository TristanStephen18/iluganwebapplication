...
async function getEmployees(uid) {
    ...
    try {
        ...
        // Add event listeners for disable/enable and delete buttons
        document.querySelectorAll(".disable-btn, .enable-btn").forEach(button => {
            button.addEventListener("click", (e) => handleToggleAccount(e.target));
        });

        document.querySelectorAll(".delete-btn").forEach(button => {
            button.addEventListener("click", (e) => handleDeleteAccount(e.target.dataset.id));
        });
...
async function handleToggleAccount(button) {
    ...
    if (confirmResult.isConfirmed) {
        try {
            const companyEmployeeRef = doc(db, `companies/${userId}/employees`, employeeId);
            ...
            if (action === "disable") {
                ...
                document.querySelector(`[data-emp-number="${employeeId}"] td:nth-child(5)`).innerText = "disabled";
            } else {
                ...
                document.querySelector(`[data-emp-number="${employeeId}"] td:nth-child(5)`).innerText = "inactive";
            }

            Swal.fire(action === "disable" ? "Disabled!" : "Enabled!", `The account has been ${action}d.`, "success");
